  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 20 }}>
      <div style={{ background: '#00000030', borderRadius: 12, boxShadow: '0 2px 12px #0001', padding: 20, minWidth: 300, maxWidth: 400, textAlign: 'center' }}>
        ...
        {loading ? (
          <p style={{ color: '#444444', marginBottom: 24 }}>Can't wait... (loading artwork)</p>
        ) : error ? (
          <>
            <p style={{ color: 'red', marginBottom: 24 }}>Failed to load artwork. Please try again.</p>
            <button onClick={fetchArtworkAgain} style={{ background: '#0074D9', color: '#fff', border: 'none', padding: '12px 28px', borderRadius: 6, fontSize: 16, cursor: 'pointer', marginTop: 2 }}>Refresh</button>
          </>
        ) : artwork ? (
